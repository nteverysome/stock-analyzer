/**
 * 填充全台灣股市數據（1,068 隻股票）
 * 數據源：FinMind API
 * 用途：初始化 Neon 數據庫
 */

import fetch from 'node-fetch';
import { query } from '../api/neon-db.js';

const BATCH_SIZE = 50; // 每批次處理 50 隻股票
const DELAY = 500; // 批次之間延遲 500ms

/**
 * 從 FinMind 獲取所有台灣股票清單
 */
async function fetchAllStocks() {
  console.log('📥 從 FinMind 獲取股票清單...');
  try {
    const res = await fetch('https://api.finmind.ai/api/v4/data?dataset=TaiwanStockInfo');
    const data = await res.json();
    
    if (data.data && Array.isArray(data.data)) {
      console.log(`✅ 獲取 ${data.data.length} 隻股票信息`);
      return data.data;
    }
  } catch (err) {
    console.error('❌ 獲取失敗:', err.message);
  }
  return [];
}

/**
 * 獲取單隻股票的最近 120 日歷史數據
 */
async function fetchStockHistory(symbol) {
  try {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 120);
    const startDateStr = startDate.toISOString().split('T')[0].replace(/-/g, '');

    const res = await fetch(
      `https://api.finmind.ai/api/v4/data?dataset=TaiwanStockPrice&data_id=${symbol}&start_date=${startDateStr}`
    );
    const data = await res.json();
    return data.data || [];
  } catch (err) {
    console.warn(`⚠️ 獲取 ${symbol} 歷史數據失敗:`, err.message);
    return [];
  }
}

/**
 * 插入股票基本信息
 */
async function insertStock(stock) {
  try {
    await query(
      `INSERT INTO tw_stocks (symbol, name, name_en, industry, sector, listed_date)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (symbol) DO UPDATE 
       SET name = $2, updated_at = NOW()`,
      [
        stock.stock_id,
        stock.stock_name,
        stock.stock_name_en || stock.stock_id,
        stock.industry_group || '其他',
        stock.industry || '其他',
        stock.listing_date || null,
      ]
    );
  } catch (err) {
    console.error(`❌ 插入 ${stock.stock_id} 失敗:`, err.message);
  }
}

/**
 * 插入股票價格歷史
 */
async function insertPriceHistory(symbol, priceData) {
  let count = 0;
  
  for (const record of priceData) {
    try {
      await query(
        `INSERT INTO tw_daily_prices 
         (symbol, price_date, open, high, low, close, volume, turnover)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        ON CONFLICT (symbol, price_date) DO NOTHING`,
        [
          symbol,
          record.date,
          record.open,
          record.high,
          record.low,
          record.close,
          record.volume || 0,
          record.money || 0,
        ]
      );
      count++;
    } catch (err) {
      // 忽略重複插入錯誤
    }
  }
  
  return count;
}

/**
 * 主函數：填充所有數據
 */
async function main() {
  console.log('🚀 開始填充全台灣股市數據...\n');
  
  const startTime = Date.now();
  const stocks = await fetchAllStocks();
  
  if (stocks.length === 0) {
    console.error('❌ 無法獲取股票清單');
    return;
  }

  let successCount = 0;
  let failCount = 0;
  let totalPrices = 0;

  // 分批處理
  for (let i = 0; i < stocks.length; i += BATCH_SIZE) {
    const batch = stocks.slice(i, i + BATCH_SIZE);
    console.log(`\n📦 批次 ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(stocks.length / BATCH_SIZE)}`);

    for (const stock of batch) {
      try {
        // 1. 插入基本信息
        await insertStock(stock);

        // 2. 獲取並插入價格歷史
        const priceData = await fetchStockHistory(stock.stock_id);
        const priceCount = await insertPriceHistory(stock.stock_id, priceData);

        console.log(`✅ ${stock.stock_id} - ${priceCount} 筆價格數據`);
        successCount++;
        totalPrices += priceCount;
      } catch (err) {
        console.error(`❌ ${stock.stock_id} - 失敗: ${err.message}`);
        failCount++;
      }
    }

    // 批次之間延遲
    if (i + BATCH_SIZE < stocks.length) {
      console.log(`⏳ 等待 ${DELAY}ms...`);
      await new Promise(r => setTimeout(r, DELAY));
    }
  }

  const duration = (Date.now() - startTime) / 1000;
  
  console.log(`
╔════════════════════════════════════════╗
║ 📊 填充完成！                           ║
╠════════════════════════════════════════╣
║ 成功股票：${successCount.toString().padStart(4)} 隻               ║
║ 失敗股票：${failCount.toString().padStart(4)} 隻               ║
║ 總計股票：${stocks.length.toString().padStart(4)} 隻               ║
║ 價格記錄：${totalPrices.toString().padStart(4)} 筆               ║
║ 執行時間：${duration.toFixed(1).padStart(6)}s                ║
╚════════════════════════════════════════╝
  `);
}

// 執行
main().catch(err => {
  console.error('致命錯誤:', err);
  process.exit(1);
});
